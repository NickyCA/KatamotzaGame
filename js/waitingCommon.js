...
function loadPlayers(response) {
  ...
  if (response.players.length === 0) {
    Common.showCommonError('Sin Datos', 'La partida ya no existe', returnIndex);
  } else {
...
export default class WaitingCommon {
  ...
  static insertGameId() {
    ...
    gameId.innerHTML = `Id de la partida: ${localStorage.getItem('gameId')}`;
  }
  ...
  static showHistoricalStats() {
    ...
    if (savedGameData !== undefined && savedGameData !== '') {
      ...
      if (localStorage.getItem('state') === 'returningToWaitingRoom') {
        ...
        Swal.fire({
          title: 'Tabla de puntos históricos',
          html: gameStatsHtml,
          width: 600,
          allowOutsideClick: false,
          allowEscapeKey: false,
