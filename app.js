...
const accessLogStream = fs.createWriteStream(path.join(__dirname, './js/log/access.log'), { flags: 'a' });
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('fromClient', (msg) => {
      ...
      if (announcement !== undefined && announcement.messageType === 'announce') {
        ...
        fs.appendFile('./js/log/msgs.csv', ` fromClient:,${JSON.stringify(msg)}\n fromServer:,${JSON.stringify(answer.serverMsg)}\n`, (error) => {
          if (error) {
            console.error('couldnt append in msgs file');
          }
        });
